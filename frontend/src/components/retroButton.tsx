const RetroButton=({text, icon, onClick, isActive, msgNo, extraClass}:{text:string, icon:string|null, onClick:()=>void, isActive:boolean, msgNo:number, extraClass:string})=>{
    return(
            <button className={`relative py-[6px] px-5 mx-2 rounded-lg cursor-pointer tracking-wide text-sm font-poppins font-semibold shadow-button text-nowrap flex flex-row justify-center items-center gap-2 ${isActive?'bg-primary text-white':'bg-background text-secondary'} ${extraClass}`}>
                {icon && <img src={icon} alt={text+"_icon"} className="object-contain h-full"/>}
                {text}
