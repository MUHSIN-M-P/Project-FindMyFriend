...
const Message = ({ message, pfp }: props) => {
    ...
    return (
        <div className="self-start-safe flex items-center gap-2">
            <img
                src={pfp || "/avatars/male_avatar.png"}
                alt="sender avatar"
                className="w-8 h-8 rounded-full border border-retro_border object-cover"
            />
            <div className="border w-fit py-2 px-3 text-secondary rounded-b-lg rounded-tr-lg my-2 shadow max-w-[65vw] md:max-w-[25vw]">
                {message.msg}
            </div>
        </div>
    );
